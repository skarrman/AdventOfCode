...
fn inside_map(pos: Pos) -> bool {
    0 <= pos.x && pos.x <= MAP_SIZE && 0 <= pos.y && pos.y <= MAP_SIZE
}

fn is_empty_after_steps(pos: Pos, fallen_bytes: &Vec<Pos>, steps: i32) -> bool {
    for i in 0..=(steps as usize) {
        if pos == fallen_bytes[i] {
            return false;
        }
    }
    true
